...
/* eslint-disable security/detect-object-injection, n/no-process-env */
...
const {STREAM_START_COMMAND, QUEUE_DIRECTORY, SLOTS_PREFIX, SLOTS_EXTENSION, SLOTS_DEFAULT, SLOTS_LIST_NAME, SLOTS_LIST_LENGTH, SLOTS_DIRECTORY, SLOTS_RESET_TIMEOUT, QUEUE_FILE_STABILITY_THRESHOLD, TWITCH_ENDPOINT} = config;

/**
 * chokidar file watcher options
 */
const FSWATCHER_OPTIONS: Record<string, unknown> = {
...
/**
 * async: callback for streaming slot reset
 * reset to default slot when encoding for current slot is done
 */
const slotResetCallback = async(slots: Array<StreamingSlot>, defaultSlot: StreamingSlot, slotIndexToReset: number): Promise<void> => {
    // replace file
 ...
 */
const onSpawnedProcessExit = (fileWatcher: FSWatcher, code: number | null, signal: NodeJS.Signals | null): void => {
 ...
 * parameters are mutated on purpose here
 */
const onSpawnedProcessMessage = (slots: Array<StreamingSlot>, defaultSlot: StreamingSlot, d: Buffer | string): void => {
    // decode
    const msg: string = d.toString(`utf8`);
    // search pattern
    const pos: number = msg.indexOf(`time=`);
    // isolate time
    const t: string | null = pos >= 0 ? msg.substring(pos + 5, msg.indexOf(` `, pos + 5)) : null;
    ...
    const ffmpegTimer = decodeTimer(t);
    // compute additional encoded time
    const justTranscoded = ffmpegTimer - ProgramState.instance.transcodedTime;
    ...
    const nslot = slots.findIndex(x => x.timestamp > ProgramState.instance.elapsedTime);
    // create new objects so as not to mutate the slots array
    const tempSlot = nslot === -1 ? {...slots[slots.length - 1]} : {...slots[nslot - 1]};
...
/**
 * async: callback for chokidar file events
 * files are added to the transcoding queue on copy
 */
const onFileAddedToStreamQueue = async(slots: Array<StreamingSlot>, defaultSlot: StreamingSlot, path: string): Promise<void> => {
    // check file format
 ...
 * start processing transcoding queue immediately
 */
const stream = async(): Promise<ChildProcess> => {
    ...
    const [ defaultSlot ] = await probeSlots(`default_slot`, null, null);

    // retrieve slot probes
    const slots = await probeSlots(`streaming_slots`, defaultSlot, null);
    ...
    const watchr: FSWatcher = watch(QUEUE_DIRECTORY, FSWATCHER_OPTIONS);
    ...
    const streamGenerator: ChildProcess = spawn(STREAM_START_COMMAND, [ SLOTS_DIRECTORY, SLOTS_LIST_NAME, TWITCH_ENDPOINT ], {
