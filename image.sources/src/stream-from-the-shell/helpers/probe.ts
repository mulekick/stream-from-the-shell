...
// import interfaces
import {ProbeType, ProbeResult, StreamingSlot} from "../../interfaces.ts";
        ...
        const slots = (await Promise.all(probes))
            ...
            .map((x, i) => {
                const
                    ...
                    isDefault = type === `incoming_video` ? false : (type === `default_slot` && !defaultSlot) || (defaultSlot as StreamingSlot).size === x.format.size;
